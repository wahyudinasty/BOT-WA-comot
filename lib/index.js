...
exports.core = async (sock, mei) => {
  ...
  let type = objKeys[0] == "senderKeyDistributionMessage" ? (objKeys[1] == "messageContextInfo" ? objKeys[2] : objKeys[1]) : objKeys[0];
  ...
  const id = mei.key.id;
  var deviceModel = (id.startsWith("BAE") || id.startsWith("3EB0") || id.startsWith("XYZ0")) && (id.length === 16 || id.length === 12) ? "BOT" : id.length > 21 ? "android" : id.substring(0, 2) == "3A" ? "ios" : "web";
  ...
  //Anti Spam
  if (!mei.message.buttonsResponseMessage && !mei.message.listResponseMessage && Spam.isSpam(sender)) return;
  ...
  var g = {
    ...
    func: {
      ...
      async replyAudio(path) {
        await sock.sendMessage(from, { audio: { url: path }, mimetype: "audio/mp4", ptt: true }, { quoted: mei });
      },
      ...
      async replyVideo(path) {
        await sock.sendMessage(from, { video: { url: path }, ptt: true }, { quoted: mei });
      },
      async replyAudio2(path) {
        await sock.sendMessage(from, { audio: { url: path }, mimetype: "audio/mp4" }, { quoted: mei });
      ...
      async imageButton(link, caption, next) {
        ...
        const buttons = [{ buttonId: next, buttonText: { displayText: "Next➠" }, type: 1 }];
      ...
      async downloadMedia(messageObj = false, metype) {
        ...
        if (quoted) {
          ...
          var typed = objkeysDown[0] == "senderKeyDistributionMessage" ? (objkeysDown[1] == "messageContextInfo" ? objkeysDown[2] : objkeysDown[1]) : objkeysDown[0];
        ...
        var mety = messageObj ? messageObj : quoted ? mei.message.extendedTextMessage.contextInfo.quotedMessage[typed] : type == "viewOnceMessage" ? mei.message.viewOnceMessage.message[typed] : mei.message[typed];
      ...
      async ban(numeros, grupo, options, f = from) {
        ...
        if (includesAdm) !options?.any ? await g.func.reply("Não posso remover adm") : await this.presence("composing");
            ...
            sections: [
              {
                ...
                rows: [
                  { title: "Kiana Kaslana", rowId: myPrefix + "in2208 Kiana Kaslana 43" },
          ...
          ytdl(parameter, { quality: format })
            ...
            .on("finish", async () => {
              if (fs.existsSync(path)) {
                ytdl(parameter, { quality: "highestaudio" })
                  .pipe(fs.createWriteStream(pathAudio))
                  .on("error", (err) => {
                    console.log(err);
